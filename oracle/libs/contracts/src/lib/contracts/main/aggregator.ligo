#include "../partials/aggregator.ligo"
...
function requestRateUpdate(const store: aggregatorStorage): return is
  block{
    ...
    if (store.deviationTriggerInfos.amount =/= 0tez) then { // -> previous round = deviation trigger
      ...
      if ( // if deviation > or < % deviation trigger
        ((store.deviationTriggerInfos.roundPrice * 1000n * 2n + store.deviationTriggerInfos.roundPrice * store.aggregatorConfig.perthousandDeviationTrigger) / (1000n * 2n) <= store.lastCompletedRoundPrice.price)
        or
        (abs(store.deviationTriggerInfos.roundPrice * 1000n * 2n - store.deviationTriggerInfos.roundPrice * store.aggregatorConfig.perthousandDeviationTrigger) / (1000n * 2n) >= store.lastCompletedRoundPrice.price)
        ) then {
    ...
    const newOracleRewardsMVK: oracleRewardsMVKType = store.oracleRewardsMVK;
  ...
  } with (operations, store with record[round=newRound; observationReveals=emptyMapReveals; observationCommits=emptyMapCommit; deviationTriggerInfos=newDeviationTriggerInfos; switchBlock=0n; oracleRewardsMVK=newOracleRewardsMVK; oracleRewardsXTZ = newOracleRewardsXTZ])
...
function requestRateUpdateDeviation(const params: setObservationCommitType; const store: aggregatorStorage): return is
  block{
    ...
    if (
    store.deviationTriggerInfos.amount =/= 0tez
    and
      (
        ((store.deviationTriggerInfos.roundPrice * 1000n * 2n + store.deviationTriggerInfos.roundPrice * store.aggregatorConfig.perthousandDeviationTrigger) / (1000n * 2n) <= store.lastCompletedRoundPrice.price)
        or
    ...
    const newOracleRewardsMVK: oracleRewardsMVKType = store.oracleRewardsMVK;
    const newOracleRewardsXTZ = Map.update(Tezos.sender, Some (getRewardAmountXTZ(Tezos.sender, store) + store.aggregatorConfig.rewardAmountXTZ), store.oracleRewardsXTZ);
  } with (operations, store with record[round=newRound; observationCommits=newObservationCommits; observationReveals=emptyMapReveals; deviationTriggerInfos=newDeviationTriggerInfos; switchBlock=0n; oracleRewardsMVK=newOracleRewardsMVK; oracleRewardsXTZ = newOracleRewardsXTZ])
...
function setObservationReveal(const params: setObservationRevealType; const store: aggregatorStorage): return is
  ...
  [@view] function lastCompletedRoundPrice (const _ : unit ; const store: aggregatorStorage) : lastCompletedRoundPriceReturnType is block {
    const withDecimal : lastCompletedRoundPriceReturnType = record [
      ...
      priceDateTime= store.lastCompletedRoundPrice.priceDateTime;
    ]
  } with (withDecimal)
...
function main (const action : action; const aggregatorStorage : aggregatorStorage) : list(operation) * aggregatorStorage is
  case action of [
