...
function getMTokenMintOrBurnEntrypoint(const tokenContractAddress : address) : contract(mintOrBurnType) is
    case (Tezos.get_entrypoint_opt(
        ...
        tokenContractAddress) : option(contract(mintOrBurnType))) of [
                Some(contr) -> contr
            |   None -> (failwith(error_MINT_OR_BURN_ENTRYPOINT_IN_MTOKEN_NOT_FOUND) : contract(mintOrBurnType))
        ]
...
// helper function to mint or burn mToken
...
block {
    ...
    const mTokenContractAddress                 : address       = createLoanTokenParams.mTokenContractAddress;
    const mTokenId                              : nat           = createLoanTokenParams.mTokenId;
    ...
    const newLoanTokenRecord : loanTokenRecordType = record [
        ...
        mTokenContractAddress               = mTokenContractAddress;
        mTokenId                            = mTokenId;
...
block {
    ...
    if collateralToken.isStakedToken then {
        ...
        // use %onVaultLiquidateStake entrypoint in Staking Contract (e.g. Doorman) to transfer staked token (e.g. smVK) balances
