...
[@view] function getGovernanceAddress(const _ : unit; const s : breakGlassStorageType) : address is
    s.governanceAddress
...
[@view] function getCouncilActionOpt(const actionId: nat; const s : breakGlassStorageType) : option(councilActionRecordType) is
    Big_map.find_opt(actionId, s.actionsLedger)



(* View: get a action signer opt *)
[@view] function getCouncilActionSignerOpt(const signerId: (nat * address); const s : breakGlassStorageType) : option(unit) is
