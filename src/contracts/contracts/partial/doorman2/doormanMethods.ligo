...
function setTotalVMvkSupply(const totalSupply : nat; var s : storage) is
block {
  s.totalVMvkSupply := totalSupply;
} with (noOperations, s);

function setTotalMvkSupply(const totalSupply : nat; var s : storage) is
block {
  s.totalMvkSupply := totalSupply;
} with (noOperations, s);
