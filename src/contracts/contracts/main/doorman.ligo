...
// helper function to update satellite's balance
function onSatelliteRewardPaid(const delegationAddress : address) : contract(unit) is
  case (Tezos.get_entrypoint_opt(
      "%onSatelliteRewardPaid",
      delegationAddress) : option(contract(unit))) of [
    Some(contr) -> contr
  | None -> (failwith(error_ON_SATELLITE_REWARD_PAID_ENTRYPOINT_IN_DELEGATION_CONTRACT_NOT_FOUND) : contract(unit))
];
