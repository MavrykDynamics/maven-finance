...
// Farm types
#include "../partials/contractTypes/farmSingleTokenTypes.ligo"
...
type return is list (operation) * farmSingleTokenStorageType
const noOperations : list (operation) = nil;

// farm contract methods lambdas
type farmSingleTokenUnpackLambdaFunctionType is (farmSingleTokenLambdaActionType * farmSingleTokenStorageType) -> return
...
function getDepositorDeposit(const depositor: depositorType; const s: farmSingleTokenStorageType): option(depositorRecordType) is
...
function checkSenderIsCouncilOrFarmFactory(const s: farmSingleTokenStorageType): unit is
block {
...
function checkSenderIsGovernanceOrFactory(const s: farmSingleTokenStorageType): unit is
block {
...
function checkFarmIsInit(const s: farmSingleTokenStorageType): unit is 
  if not s.init then failwith(error_FARM_NOT_INITIATED)
  else unit



function checkFarmIsOpen(const s: farmSingleTokenStorageType): unit is 
...
function checkDepositIsNotPaused(var s : farmSingleTokenStorageType) : unit is
    if s.breakGlassConfig.depositIsPaused then failwith(error_DEPOSIT_ENTRYPOINT_IN_FARM_CONTRACT_PAUSED)
    else unit;

function checkWithdrawIsNotPaused(var s : farmSingleTokenStorageType) : unit is
    if s.breakGlassConfig.withdrawIsPaused then failwith(error_WITHDRAW_ENTRYPOINT_IN_FARM_CONTRACT_PAUSED)
    else unit;

function checkClaimIsNotPaused(var s : farmSingleTokenStorageType) : unit is
...
function transferReward(const depositor: depositorType; const tokenAmount: tokenBalanceType; const s: farmSingleTokenStorageType): operation is
block{
...
function updateBlock(var s: farmSingleTokenStorageType): farmSingleTokenStorageType is
block{
...
function updateFarmParameters(var s: farmSingleTokenStorageType): farmSingleTokenStorageType is
block{
    ...
    // Updates the farmSingleTokenStorageType
...
function updateFarm(var s: farmSingleTokenStorageType): farmSingleTokenStorageType is
block{
...
function updateUnclaimedRewards(const depositor: depositorType; var s: farmSingleTokenStorageType): farmSingleTokenStorageType is
block{
    ...
    // Update paid and unpaid rewards in farmSingleTokenStorageType
...
function unpackLambda(const lambdaBytes : bytes; const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType; var s : farmSingleTokenStorageType) : return is 
block {

    const res : return = case (Bytes.unpack(lambdaBytes) : option(farmSingleTokenUnpackLambdaFunctionType)) of [
        Some(f) -> f(farmSingleTokenLambdaAction, s)
...
// Farm Single Token Lambdas:
#include "../partials/contractLambdas/farmSingleToken/farmSingleTokenLambdas.ligo"
...
[@view] function getAdmin(const _: unit; var s : farmSingleTokenStorageType) : address is
  s.admin



(* View: get name variable *)
[@view] function getName(const _: unit; var s : farmSingleTokenStorageType) : string is
  s.name



(*  View: get config *)
[@view] function getConfig(const _: unit; const s: farmSingleTokenStorageType) : farmConfigType is
  s.config



(*  View: get whitelist contracts *)
[@view] function getWhitelistContracts(const _: unit; const s: farmSingleTokenStorageType) : whitelistContractsType is
  s.whitelistContracts



(*  View: get general contracts *)
[@view] function getGeneralContracts(const _: unit; const s: farmSingleTokenStorageType) : generalContractsType is
  s.generalContracts



(*  View: get break glass config *)
[@view] function getBreakGlassConfig(const _: unit; const s: farmSingleTokenStorageType) : farmBreakGlassConfigType is
  s.breakGlassConfig



(*  View: get last block update *)
[@view] function getLastBlockUpdate(const _: unit; const s: farmSingleTokenStorageType) : nat is
  s.lastBlockUpdate



(*  View: get last block update *)
[@view] function getAccumulatedRewardsPerShare(const _: unit; const s: farmSingleTokenStorageType) : nat is
  s.accumulatedRewardsPerShare



(*  View: get claimed rewards *)
[@view] function getClaimedRewards(const _: unit; const s: farmSingleTokenStorageType) : claimedRewardsType is
  s.claimedRewards



(*  View: get depositor *)
[@view] function getDepositorOpt(const depositorAddress: depositorType; const s: farmSingleTokenStorageType) : option(depositorRecordType) is
  Big_map.find_opt(depositorAddress, s.depositors)



(*  View: get open *)
[@view] function getOpen(const _: unit; const s: farmSingleTokenStorageType) : bool is
  s.open



(*  View: get init *)
[@view] function getInit(const _: unit; const s: farmSingleTokenStorageType) : bool is
  s.init



(*  View: get init block *)
[@view] function getInitBlock(const _: unit; const s: farmSingleTokenStorageType) : nat is
  s.initBlock



(* View: get a lambda *)
[@view] function getLambdaOpt(const lambdaName: string; var s : farmSingleTokenStorageType) : option(bytes) is
  Map.find_opt(lambdaName, s.lambdaLedger)



(* View: get the lambda ledger *)
[@view] function getLambdaLedger(const _: unit; var s : farmSingleTokenStorageType) : lambdaLedgerType is
  s.lambdaLedger
...
(*  setAdmin entrypoint *)
function setAdmin(const newAdminAddress : address; var s : farmSingleTokenStorageType) : return is
block {
    ...
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaSetAdmin(newAdminAddress);

    // init response
    const response : return = unpackLambda(lambdaBytes, farmSingleTokenLambdaAction, s);  
...
function setGovernance(const newGovernanceAddress : address; var s : farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaSetGovernance(newGovernanceAddress);
...
function setName(const updatedName : string; var s : farmSingleTokenStorageType) : return is
block {
    ...
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaSetName(updatedName);
...
function updateMetadata(const updateMetadataParams : updateMetadataType; var s : farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUpdateMetadata(updateMetadataParams);
...
function updateConfig(const updateConfigParams : farmUpdateConfigParamsType; var s : farmSingleTokenStorageType) : return is 
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUpdateConfig(updateConfigParams);
...
function updateWhitelistContracts(const updateWhitelistContractsParams: updateWhitelistContractsType; var s: farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUpdateWhitelistContracts(updateWhitelistContractsParams);
...
function updateGeneralContracts(const updateGeneralContractsParams: updateGeneralContractsType; var s: farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUpdateGeneralContracts(updateGeneralContractsParams);
...
function mistakenTransfer(const destinationParams: transferActionType; var s: farmSingleTokenStorageType): return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaMistakenTransfer(destinationParams);
...
function updateBlocksPerMinute(const blocksPerMinute: nat; var s: farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUpdateBlocksPerMinute(blocksPerMinute);
...
(* initFarm Entrypoint *)
function initFarm (const initFarmParams: initFarmParamsType; var s: farmSingleTokenStorageType) : return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaInitFarm(initFarmParams);
...
function closeFarm (var s: farmSingleTokenStorageType) : return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaCloseFarm(unit);
...
(*  pauseAll entrypoint *)
function pauseAll(var s: farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaPauseAll(unit);
...
function unpauseAll(var s : farmSingleTokenStorageType) : return is
block {
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaUnpauseAll(unit);
...
function togglePauseEntrypoint(const targetEntrypoint: farmTogglePauseEntrypointType; const s: farmSingleTokenStorageType): return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaTogglePauseEntrypoint(targetEntrypoint);
...
(* deposit Entrypoint *)
function deposit(const tokenAmount: tokenBalanceType; var s: farmSingleTokenStorageType) : return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaDeposit(tokenAmount);
...
(* withdraw Entrypoint *)
function withdraw(const tokenAmount: tokenBalanceType; var s: farmSingleTokenStorageType) : return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaWithdraw(tokenAmount);
...
(* Claim Entrypoint *)
function claim(const depositor: depositorType; var s: farmSingleTokenStorageType) : return is
block{
    ...
    // init farm lambda action
    const farmSingleTokenLambdaAction : farmSingleTokenLambdaActionType = LambdaClaim(depositor);
...
function setLambda(const setLambdaParams: setLambdaType; var s: farmSingleTokenStorageType): return is
block{
...
(* main entrypoint *)
function main (const action : farmSingleTokenAction; var s: farmSingleTokenStorageType): return is
  block{
