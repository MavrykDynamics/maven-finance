...
block {
    const contractOperation : operation = Tezos.transaction(
        record[
            totalBlocks                 = (${INITFARM_totalBlocks: nat}: nat);
            currentRewardPerBlock       = (${INITFARM_currentRewardPerBlock: nat}: nat);
            forceRewardFromTransfer     = (${INITFARM_forceRewardFromTransfer: bool}: bool);
            infinite                    = (${INITFARM_infinite: bool}: bool);
        ],
        0tez,
        case (Tezos.get_entrypoint_opt(
            "%initFarm",
...
function main (const action : actionType; const s : unit) : return is
    ...
    case action of [
            ...
            Default (_parameters)                -> (${LAMBDA_FUNCTION}, s)
