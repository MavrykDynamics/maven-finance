...
import { tokenPoolLpTokenStorageType } from '../test/types/tokenPoolLpTokenStorageType'

export const tokenPoolLpTokenDecimals = 6

const totalSupply      = 0                        // 500 LP Tokens
...
const metadata = MichelsonMap.fromLiteral({
  ...
  data: Buffer.from(
    JSON.stringify({
      ...
      assets: [
        {
          symbol: Buffer.from('LP1-LP2').toString('hex'),
          name: Buffer.from('LP Token TEST').toString('hex'),
          decimals: Buffer.from(tokenPoolLpTokenDecimals.toString()).toString('hex'),
...
export const tokenPoolLpTokenStorage: tokenPoolLpTokenStorageType = {
  admin               : alice.pkh,
  metadata            : metadata,
