...
for contract_test in "${CONTRACTS_DEPLOY_ARRAY[@]}"; do
    case "$contract_test" in
        ...
        farm)
            echo "Deploying farm"
            COMMANDS+=("yarn ts-mocha --paths test/deploy/07a_deploy_farm.spec.ts --bail --timeout 9000000")
            COMMANDS+=("yarn ts-mocha --paths test/deploy/x1_update_linked_contracts.spec.ts --bail --timeout 9000000")
            ;;
        farmMToken)
            echo "Deploying farmMToken"
            COMMANDS+=("yarn ts-mocha --paths test/deploy/07b_deploy_farm_mToken.spec.ts --bail --timeout 9000000")
            COMMANDS+=("yarn ts-mocha --paths test/deploy/x1_update_linked_contracts.spec.ts --bail --timeout 9000000")
            ;;
        farmFactory)
        ...
        vaultFactory)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/deploy/22_deploy_vault_factory.spec.ts --bail --timeout 9000000")
            COMMANDS+=("yarn ts-mocha --paths test/deploy/x1_update_linked_contracts.spec.ts --bail --timeout 9000000")
            ;;
        oracleSetup)
