...
for contract_test in "${CONTRACTS_TEST_ARRAY[@]}"; do
    case "$contract_test" in
        mvkToken)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/01_test_mvk_token.spec.ts --bail --timeout 300000")
            ;;
        doorman)
            echo "Running tests for doorman"
            COMMANDS+=("yarn ts-mocha --paths test/02_test_doorman.spec.ts --bail --timeout 300000")
            ;;
        delegation)
            echo "Running tests for delegation"
            COMMANDS+=("yarn ts-mocha --paths test/03a_test_delegation.spec.ts --bail --timeout 300000")
            COMMANDS+=("yarn ts-mocha --paths test/03b_test_delegation.spec.ts --bail --timeout 300000")
            ;;
        governance)
        ...
        breakGlass)
            ...
            COMMANDS+=("yarn ts-mocha --paths test/11_test_break_glass.spec.ts --bail --timeout 300000")
            ;;
        governanceFinancial)
            echo "Running tests for governanceFinancial"
            COMMANDS+=("yarn ts-mocha --paths test/12_test_governance_financial.spec.ts --bail --timeout 300000")
            ;;
        governanceFinancial)
            echo "Running tests for governanceFinancial"
            COMMANDS+=("yarn ts-mocha --paths test/21_test_governance_financial.spec.ts --bail --timeout 300000")
            ;;
        governanceProxy)
            echo "Running tests for governanceProxy"
            COMMANDS+=("yarn ts-mocha --paths test/13_test_governance_proxy.spec.ts --bail --timeout 300000")
            ;;
        superAdmin)
            echo "Running tests for superAdmin"
            COMMANDS+=("yarn ts-mocha --paths test/14_test_super_admin.spec.ts --bail --timeout 300000")
            ;;
        votingPowerRatio)
            echo "Running tests for votingPowerRatio"
            COMMANDS+=("yarn ts-mocha --paths test/15_test_voting_power_ratio.spec.ts --bail --timeout 300000")
            ;;
        mistakenTransfers)
            echo "Running tests for mistakenTransfers"
            COMMANDS+=("yarn ts-mocha --paths test/16_test_mistaken_transfers.spec.ts --bail --timeout 300000")
            ;;
        governanceQuorum)
            echo "Running tests for governanceQuorum"
            COMMANDS+=("yarn ts-mocha --paths test/17_test_governance_quorum.spec.ts --bail --timeout 300000")
            ;;
        aggregator)
            echo "Running tests for aggregator"
            COMMANDS+=("yarn ts-mocha --paths test/18_test_aggregator.spec.ts --bail --timeout 300000")
            ;;
        aggregatorFactory)
            echo "Running tests for aggregatorFactory"
            COMMANDS+=("yarn ts-mocha --paths test/19_test_aggregator_factory.spec.ts --bail --timeout 300000")
            ;;
        satelliteStatus)
            echo "Running tests for satelliteStatus"
            COMMANDS+=("yarn ts-mocha --paths test/22_test_satellite_status.spec.ts --bail --timeout 300000")
            ;;
        stressTest)
            echo "Running tests for stressTest"
            COMMANDS+=("yarn ts-mocha --paths test/23_test_stress_test.spec.ts --bail --timeout 300000")
            ;;
        mavrykTokens)
            echo "Running tests for mavrykTokens"
            COMMANDS+=("yarn ts-mocha --paths test/25_test_mavryk_tokens.spec.ts --bail --timeout 300000")
            ;;
        lendingController)
            echo "Running tests for lendingController"
            COMMANDS+=("yarn ts-mocha --paths test/26_test_lending_controller.spec.ts --bail --timeout 300000")
            ;;
        lendingControllerPause)
            echo "Running tests for lendingControllerPause"
            COMMANDS+=("yarn ts-mocha --paths test/27_test_lending_controller_pause.spec.ts --bail --timeout 300000")
            ;;
        lendingControllerMockTimeMonth)
            echo "Running tests for lendingControllerMockTime - Month"
            COMMANDS+=("yarn ts-mocha --paths test/28_test_lending_controller_mock_time_month.spec.ts --bail --timeout 300000")
            ;;
        lendingControllerMockTimeYear)
            echo "Running tests for lendingControllerMockTime - Year"
            COMMANDS+=("yarn ts-mocha --paths test/29_test_lending_controller_mock_time_year.spec.ts --bail --timeout 300000")
            ;;
        lendingControllerMockTimeLiquidation)
            echo "Running tests for lendingControllerMockTime - Liquidation"
            COMMANDS+=("yarn ts-mocha --paths test/30_test_lending_controller_mock_time_liquidation.spec.ts --bail --timeout 300000")
            ;;
        mToken)
            echo "Running tests for mToken"
            COMMANDS+=("yarn ts-mocha --paths test/31_test_mToken.spec.ts --bail --timeout 300000")
            ;;
        vault)
            echo "Running tests for vault"
            COMMANDS+=("yarn ts-mocha --paths test/32_test_vault.spec.ts --bail --timeout 300000")
            ;;
        all)
