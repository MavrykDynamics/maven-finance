...
describe("Test: Doorman Contract", async () => {
    ...
    describe("%stake", async () => {
        it("user should not be able to stake less than 1MVK", async() => {
            try{
                ...
                // Operation
                stakeOperation = await doormanInstance.methods.stake(userStake);
    ...
    describe("%unstake", async () => {
        it("user should not be able to unstake less than 1MVK", async() => {
            try{
                ...
                // Initial values
                const userUnstake = MVK(0.1);

                // Operation
                unstakeOperation = await doormanInstance.methods.unstake(userUnstake);
        ...
        it("user should not be able to unstake more MVK than he has staked before", async() => {
            try{
                ...
                const userStakeLedger   = await doormanStorage.userStakeBalanceLedger.get(bob.pkh);
                const userStakeBalance  = userStakeLedger === undefined ? 0 : userStakeLedger.balance.toNumber()
                const userUnstake       = userStakeBalance +  MVK(1);
        ...
        it("single user should be able to unstake some MVK and earn all the exit fee", async() => {
            try{
                ...
                // Operation
                unstakeOperation = await doormanInstance.methods.unstake(userUnstake).send();
                ...
                // Assertion
                assert.equal(helperFunctions.almostEqual(Math.floor(doormanSMVKTotalSupply - expectedFinalAmount), doormanSMVKTotalSupplyEnd, 0.01), true)
                assert.equal(helperFunctions.almostEqual(Math.round(userMVKBalance + expectedFinalAmount), userMVKBalanceEnd, 0.01), true)
                assert.equal(helperFunctions.almostEqual(Math.floor(userStakeBalance - expectedFinalAmount), userStakeBalanceEnd, 0.01), true)
        ...
        it("multiple users should be able to unstake some MVK and share the exit fee", async() => {
            try{
                ...
                const mvkTotalSupply        = mvkTokenStorage.totalSupply.toNumber()
                const smvkTotalSupply       = ((await mvkTokenStorage.ledger.get(contractDeployments.doorman.address)) === undefined ? new BigNumber(0) : (await mvkTokenStorage.ledger.get(contractDeployments.doorman.address))).toNumber();
    ...
    describe("%compound", async () => {
        ...
        it("user should not see any increase in rewards when compounding twice in quick succession", async() => {
            try{
    ...
    describe("%migrateFunds", async () => {
        ...
        it("Admin should be able to migrate the Doorman contract MVK funds", async() => {
            try{
                ...
                const endDoormanBalance     = await mvkTokenStorage.ledger.get(contractDeployments.doorman.address);
