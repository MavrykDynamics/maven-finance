...
describe("Governance proxy lambdas tests", async () => {
    ...
    describe("%createFarm", async() => {
        ...
        //             "FirstFarm#3": packedParam,
        //             "FirstFarm#4": packedParam,
        //             "FirstFarm#5": packedParam,
        //             "FirstFarm#6": packedParam,
        //         });
    ...
    //                 "FirstTreasury#3": packedParam,
    //                 "FirstTreasury#4": packedParam,
    //                 "FirstTreasury#5": packedParam
    //             });
    ...
    describe("%updateContractWhitelistTokenMap", async() => {
        ...
        it("Scenario - Add a new token to the treasury factory contract whitelist tokens map", async() => {
            try{
                ...
                governanceStorage           = await governanceInstance.storage();
                treasuryFactoryStorage      = await treasuryFactoryInstance.storage();
                const initWhitelist         = treasuryFactoryStorage.whitelistTokenContracts;
                const proposalId            = governanceStorage.nextProposalId.toNumber();
                ...
                governanceStorage           = await governanceInstance.storage();
                delegationStorage           = await delegationInstance.storage();
                const endWhitelist          = delegationStorage.whitelistTokenContracts;
                const proposal              = await governanceStorage.proposalLedger.get(proposalId);
    ...
    describe("%updateDoormanMinMVKAmount", async() => {
        ...
        it("Scenario - Update the doorman minMvkAmount", async() => {
            try{
                ...
                assert.equal(endAmount, MVK(0.25));
            } catch(e) {
                console.dir(e, {depth:5})
            }
        })
    ...
    describe("%initFarm", async() => {
        ...
        it("Scenario - Initialize a farm", async() => {
            try{
                ...
                assert.equal(endConfig.plannedRewards.rewardPerBlock, MVK(100));
    ...
    describe("%updateContractLambda", async() => {
        beforeEach("Set signer to admin", async() => {
            await signerFactory(bob.sk)
        })

        it("Scenario - Update the unstake entrypoint of the doorman contract", async() => {
            try{
                // Initial values
                governanceStorage                   = await governanceInstance.storage();
                doormanStorage                      = await doormanInstance.storage();
                ...
                if (referenceDataPacked) {
                    packedParam = referenceDataPacked.packed
                    console.log('packed %updateContractLambda param: ' + packedParam);
                } else {
                    throw `packing failed`
                };

                const proposalMetadata      = MichelsonMap.fromLiteral({
                    "UpdateContractLambda#1": packedParam
                });
