...
describe('Linked contracts updates for Tests', async () => {
  ...
  before('setup', async () => {
    try{
      ...
      // const governanceSatelliteContractOperation = (await governanceSatelliteInstance.methods.updateWhitelistContracts("aggregatorFactory", aggregatorFactoryAddress.address).send()) as TransactionOperation
      ...
      .withContractCall(governanceInstance.methods.updateGeneralContracts('lendingControllerMockTime', lendingControllerMockTimeAddress.address))
