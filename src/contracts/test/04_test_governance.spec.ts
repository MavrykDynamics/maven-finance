        ...
        describe("%updateProposalData", async () => {
            ...
            it('Satellite should not be able to call this entrypoint if it is not the proposal round', async () => {
                try{
                    ...
                    // add proposal data
                    const sampleConfigParam = governanceProxyInstance.methods.dataPackingHelper(
                        'updateCouncilConfig',
                        1234,
                        'configActionExpiryDays'
                    ).toTransferParams();
                    const sampleConfigValue = sampleConfigParam.parameter.value;
                    const dataPackingHelperEntrypointType = await governanceProxyInstance.entrypoints.entrypoints.dataPackingHelper;
        
                    const sampleConfigPackedValue = await utils.tezos.rpc.packData({
                        data: sampleConfigValue,
                        type: dataPackingHelperEntrypointType
                    }).catch(e => console.error('error:', e));
        
                    var samplePackedData;
                    if (sampleConfigPackedValue) {
                        samplePackedData = sampleConfigPackedValue.packed
                    } else {
                        throw `packing failed`
                    };
