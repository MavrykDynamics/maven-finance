...
describe("Lending Controller tests", async () => {
    ...
    describe('%repay', function () {
        ...
        beforeEach("set mock block level", async () => {

            await signerFactory(bob.sk);

            const oneYearLevelBlocks = 4320;

            const setMockLevelOperation = await lendingControllerInstance.methods.updateConfig('configMockLevel', oneYearLevelBlocks).send();
            await setMockLevelOperation.confirmation();

            const updatedLendingControllerStorage = await lendingControllerInstance.storage();
            const updatedMockLevel = updatedLendingControllerStorage.config.mockLevel;

            assert.equal(updatedMockLevel, oneYearLevelBlocks);
