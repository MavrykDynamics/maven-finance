    ...
    describe("%distributeRewards", async () => {
        ...
        it('Reward distribution tests #2', async () => {
            try{
                ...
                await signerFactory(alice.sk);
                const delegateOperation = await delegationInstance.methods.delegateToSatellite(mallory.pkh).send();
        ...
        it('End of governance cycle should trigger this entrypoint', async () => {
            try{
                // Initial Values
                delegationStorage           = await delegationInstance.storage();
                doormanStorage              = await doormanInstance.storage();
                ...
                // Assertions
                assert.equal(firstSatelliteRecordNoClaim.unpaid.toNumber(), 0)
                assert.equal(firstSatelliteRecordNoClaim.paid.toNumber(), firstSatelliteRecordStart.paid.toNumber() + firstSatelliteFee + firstSatelliteAccu * firstSatelliteStakeStart.balance.toNumber())
                assert.equal(secondSatelliteRecordNoClaim.unpaid.toNumber(), 0)
                assert.equal(secondSatelliteRecordNoClaim.paid.toNumber(), secondSatelliteRecordStart.paid.toNumber() + secondSatelliteFee + secondSatelliteAccu * secondSatelliteStakeStart.balance.toNumber())
