...
//             const batchSize             = 50
//             const tezAmount             = 5
//             const userAmount            = randomUserAccounts.length + 1;
//             const mvkAmount             = mainUserMVKBalance.div(userAmount);
...
//     //                 const acccountLPAllowances  = await accountLPRecord.allowances.get(farmAddress.address);
...
//     //                     depositBatch.withContractCall(lpTokenInstance.methods.approve(farmAddress.address, approvals))
//     //                 }
...
//                         const batchOpEstimate       = await utils.tezos.estimate.batch([
//                             { kind: OpKind.TRANSACTION, to: mvkTokenAddress.address, parameter: updateOperatorsParams.parameter, amount: 0},
//                             { kind: OpKind.TRANSACTION, to: doormanAddress.address, parameter: stakeParams.parameter, amount: 0},
//                         ])
...
//                     const exitFee           = 500 / (mli + 5);
//                     const paidFee           = amountToUnstake * exitFee / 100;
