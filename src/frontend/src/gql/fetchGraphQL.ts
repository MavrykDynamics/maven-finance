...
async function fetchGraphQL(operationsDoc: string, operationName: string, variables: Record<string, any>) {
  ...
  const gqlAPINetwork =
    process.env.NODE_ENV === 'development' ? process.env.REACT_APP_DEV_GRAPHQL_API : process.env.REACT_APP_GRAPHQL_API
  return new Promise<any>((resolve, reject) => {
    fetch(gqlAPINetwork || 'https://api.mavryk.finance/v1/graphql', {
      method: 'POST',
      headers: {
        'Content-type': 'application/json',
        Accept: 'application/json',
