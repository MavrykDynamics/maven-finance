...
export interface EmergencyGovernanceState {
  type?:
    | typeof GET_EMERGENCY_GOVERNANCE_STORAGE
    | typeof SUBMIT_EMERGENCY_GOVERNANCE_PROPOSAL_REQUEST;
...
export function emergencyGovernance(
  state = emergencyGovernanceDefaultState,
  action: any
): EmergencyGovernanceState {
  switch (action.type) {
    ...
    case SET_HAS_ACKNOWLEDGED_EMERGENCY_GOV:
      return {
        ...state,
        hasAcknowledgeEmergencyGovernance:
          action.hasAcknowledgeEmergencyGovernance,
      };
