...
export const getMvkTokenStorage =
...
export const stake =
  (amount: number) => async (dispatch: any, getState: any) => {
    ...
    if (!(amount > 0)) {
      dispatch(
        showToaster(
          ERROR,
          "Incorrect amount",
          "Please enter an amount superior to zero"
        )
      );
      return;
    }
    ...
    try {
      const mvkTokenContract = await state.wallet.tezos?.wallet.at(
        state.contractAddresses.mvkTokenAddress.address
      );
      const doormanContract = await state.wallet.tezos?.wallet.at(
        state.contractAddresses.doormanAddress.address
      );
      ...
      const batch =
        ...
        (await state.wallet.tezos?.wallet
          .batch()
          .withContractCall(
            mvkTokenContract.methods.update_operators(addOperators)
          )
          .withContractCall(
            doormanContract.methods.stake(amount * PRECISION_NUMBER)
          )
          .withContractCall(
            mvkTokenContract.methods.update_operators(removeOperators)
          ));
      ...
      if (state.wallet.accountPkh)
        dispatch(getUserData(state.wallet.accountPkh));
...
export const unstake =
  (amount: number) => async (dispatch: any, getState: any) => {
    ...
    try {
      const contract = await state.wallet.tezos?.wallet.at(
        state.contractAddresses.doormanAddress.address
      );
      console.log("contract", contract);
      const transaction = await contract?.methods
        .unstake(amount * PRECISION_NUMBER)
        .send();
...
export const rewardsCompound =
  (address: string) => async (dispatch: any, getState: any) => {
    const state: State = getState();
...
export const getDoormanStorage =
  (accountPkh?: string) => async (dispatch: any, getState: any) => {
    const state: State = getState();
...
export const getUserData =
  (accountPkh: string) => async (dispatch: any, getState: any) => {
    ...
    try {
      ...
      const userIsDelegatedToSatellite =
        userInfoData?.delegation_records.length > 0;
      const userInfo: UserData = {
        ...
        participationFeesPerShare: calcWithoutPrecision(
          userInfoData?.participation_fees_per_share
        ),
...
export const updateUserData =
  (field: string, value: any) => async (dispatch: any, getState: any) => {
    const state: State = getState();
    try {
