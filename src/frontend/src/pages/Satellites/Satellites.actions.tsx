...
export const getDelegationStorage =
  () => async (dispatch: any, getState: any) => {
    try {
      ...
      const delegationStorage = normalizeDelegationStorage(
        delegationStorageFromIndexer?.delegation[0]
      );
...
export const delegate =
  (satelliteAddress: string) => async (dispatch: any, getState: any) => {
    ...
    if (
      state.user.user.myMvkTokenBalance === 0 &&
      state.user.user.mySMvkTokenBalance === 0
    ) {
      dispatch(
        showToaster(ERROR, "Unable to Delegate", "Please buy MVK and stake it")
      );
      return;
    }
    ...
    if (state.user.user.mySMvkTokenBalance === 0) {
      dispatch(
        showToaster(ERROR, "Unable to Delegate", "Please stake your MVK")
      );
      return;
    }
    ...
    try {
      const contract = await state.wallet.tezos?.wallet.at(
        state.contractAddresses.delegationAddress.address
      );
      console.log("contract", contract);
      const transaction = await contract?.methods
        .delegateToSatellite(state.wallet.accountPkh, satelliteAddress)
        .send();
...
export const undelegate = () => async (dispatch: any, getState: any) => {
  ...
  try {
    ...
    console.log("contract", contract);
    const transaction = await contract?.methods
      .undelegateFromSatellite(state.wallet.accountPkh)
      .send();
    console.log("transaction", transaction);
...
export const registerFeedAction =
  () => async (dispatch: any, getState: any) => {
    const state: State = getState();
