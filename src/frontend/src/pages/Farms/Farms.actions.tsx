...
export const getFarmsContracts = () => async (dispatch: any, getState: any) => {
  ...
  const urls = farmStorage.map(
    (item) =>
      `${process.env.REACT_APP_RPC_TZKT_API}/v1/contracts/${item.lpTokenAddress}`
  ) as string[];

  try {
    const farmContracts = (await Promise.all(
...
export const harvest =
  (farmAddress: string) => async (dispatch: any, getState: any) => {
    const state: State = getState();
...
export const deposit =
  (farmAddress: string, amount: number) =>
  async (dispatch: any, getState: any) => {
    ...
    try {
      ...
      const transaction = await contract?.methods
        .deposit(amount * PRECISION_NUMBER)
        .send();
      console.log("transaction", transaction);
      ...
      if (state.wallet.accountPkh)
        await dispatch(getUserData(state.wallet.accountPkh));
...
export const withdraw =
  ...
  async (dispatch: any, getState: any) => {
    ...
    try {
      ...
      console.log("contract", contract);
      const transaction = await contract?.methods
        .withdraw(amount * PRECISION_NUMBER)
        .send();
      console.log("transaction", transaction);
