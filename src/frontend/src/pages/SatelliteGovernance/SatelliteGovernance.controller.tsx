...
const getOngoingActionsList = (
  list: GovernanceSatelliteItem
): GovernanceSatelliteItem => {
...
export const SatelliteGovernance = () => {
  ...
  const { governanceSatelliteStorage } = useSelector(
  ...
  const userIsSatellite = checkIfUserIsSatellite(
    accountPkh,
  ...
  const governanceSatelliteActionRecord =
    governanceSatelliteStorage.governance_satellite_action_record;

  const ongoingActionsAmount = getOngoingActionsList(
    governanceSatelliteActionRecord
  ).length;

  const [separateRecord, setSeparateRecord] = useState<
  ...
  useEffect(() => {
    const filterOngoing = getOngoingActionsList(
      governanceSatelliteActionRecord
    );
  ...
  const listName = useMemo(
    () => getSatelliteGovernanceListName(activeTab),
    [activeTab]
  );
    ...
    <Page>
      ...
      {paginatedItemsList?.length
        ? paginatedItemsList.map((item: any) => {
            const linkAddress =
              item.governance_satellite_action_parameters?.[0]?.value || "";
