...
4. In it, replace the value in:
   ...
   - `dipdup.extraConfig` to the config from your updated [dipdup.yml](../../../indexer/dipdup.yml) (you only need to copy/paste the lines starting from line 39 - `contracts:`). Don't forget to properly indent your yaml accordingly.
...
6. Generate the final manifest depending on the environment you want to update:
   - _PROD_: `helm template prod . --values prod.values.yaml --namespace mavryk-indexer > temp.yaml`
   - _DEV_: `helm template dev . --values dev.values.yaml --namespace mavryk-indexer > temp.yaml`
7. `kubectl apply -f ./temp.yaml` to apply the manifest. The ressources should be updating.
