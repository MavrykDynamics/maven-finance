...
async def on_vault_deposit_stake(
    ...
    try:
        ...
        # Vault
        vault                           = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=vault_address)
        ...
        # Get doorman info
        doorman_user        = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=doorman_address)
