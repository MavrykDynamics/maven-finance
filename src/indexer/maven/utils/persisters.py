...
async def persist_council_action(ctx, action):
    ...
    for council_action_id in council_action_ledger:
        ...
        initiator                       = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=council_action_initiator)
        ...
        if not action_exists:
            ...
            for council_action_signer in council_action_signers:
                ...
                if action_id == council_action_id:
                    # Signers
                    user                            = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=action_signer)
...
async def persist_break_glass_action(ctx, action):
    ...
    for break_glass_action_id in break_glass_action_ledger:
        ...
        initiator       = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=break_glass_action_initiator)
...
async def persist_financial_request(ctx, action):
    ...
    for request_id in request_ledger:
        ...
        if not request_exists:
            ...
            requester               = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=requester_address)
...
async def persist_governance_satellite_action(ctx, action):
    ...
    for action_id in action_ledger:
        ...
        if not action_exists:
            ...
            initiator                       = await models.maven_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=initiator_address)
