...
from mavryk.types.mvk_token.tezos_parameters.update_operators import UpdateOperatorsParameter
import mavryk.models as models

async def update_operators(
    ctx: HandlerContext,
    update_operators: TzktTransaction[UpdateOperatorsParameter, MvkTokenStorage],
) -> None:
    ...
    try:
        ...
        for operatorChange in operator_changes:
            if hasattr(operatorChange, 'add_operator'):
                ...
                owner               = await models.mavryk_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=owner_address)            
                operator            = await models.mavryk_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=operator_address)
