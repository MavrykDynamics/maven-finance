...
from mavryk.types.token_sale.parameter.claim_tokens import ClaimTokensParameter
from mavryk.types.token_sale.storage import TokenSaleStorage
from dipdup.context import HandlerContext
import mavryk.models as models
from dateutil import parser

async def on_token_sale_claim_tokens(
    ctx: HandlerContext,
    claim_tokens: Transaction[ClaimTokensParameter, TokenSaleStorage],
) -> None:

    try:
        # Get operation values
        token_sale_address  = claim_tokens.data.target_address
        buyer_address       = claim_tokens.data.sender_address
        buy_record_storage  = claim_tokens.storage.tokenSaleLedger[buyer_address]
