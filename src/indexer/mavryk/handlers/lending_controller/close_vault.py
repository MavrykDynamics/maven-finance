...
from mavryk.types.lending_controller.tezos_parameters.close_vault import CloseVaultParameter
...
async def close_vault(
    ...
    try:
        ...
        owner                       = await models.mavryk_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=vault_owner_address)
