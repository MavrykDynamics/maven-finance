...
async def on_vault_liquidate_stake(
    ...
    try:
        ...
        # Vault
        vault                           = await models.mavryk_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=vault_address)
        ...
        # Get doorman info
        doorman_user        = await models.mavryk_user_cache.get(network=ctx.datasource.name.replace('tzkt_',''), address=doorman_address)
